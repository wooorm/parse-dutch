...
function mergeDutchPrefixExceptions(child, index, parent) {
  ...
  if (
    ...
    if (
      ...
      prev.type === 'WordNode' &&
      toString(node) === '.' &&
      EXPRESSION_ABBREVIATION_DUTCH_PREFIX.test(toString(prev).toLowerCase())
    ) {
...
function mergeDutchElisionExceptions(child, index, parent) {
  ...
  if (!EXPRESSION_APOSTROPHE.test(toString(child))) {
    return;
  }
  ...
  if (
    ...
    if (
      EXPRESSION_ELISION_DUTCH_AFFIX.test(toString(node).toLowerCase())
    ) {
   ...
   * is not a word... */
    ...
    if (EXPRESSION_ELISION_DUTCH_PREFIX.test(toString(node).toLowerCase())) {
      /* Remove the apostrophe from parent. */
      siblings.splice(index, 1);
